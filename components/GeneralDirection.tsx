        ...
        <div className="p-4 border-b border-[#ceadd4] flex-shrink-0 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={async () => {
                const lines = [
                  `PROJECT: ${value.projectName || 'Untitled'}`,
                  `${'='.repeat(50)}`,
                  '',
                  `SETTINGS`,
                  `Video Type: ${value.videoType}  |  Format: ${value.format}  |  Duration: ${value.duration}  |  Tone: ${value.tone}`,
                  '',
                  `USER DIRECTION`,
                  value.aim || '(not set)',
                  '',
                  `CALL TO ACTION: ${value.cta || '(not set)'}`,
                  `TARGET AUDIENCE: ${value.targetAudience || '(not set)'}`,
                  '',
                  value.characterConsistency ? `CHARACTER NOTES: ${value.characterConsistency}` : '',
                  value.sceneryConsistency ? `SCENERY NOTES: ${value.sceneryConsistency}` : '',
                  value.additionalNotes ? `ADDITIONAL NOTES: ${value.additionalNotes}` : '',
                  '',
                  `${'─'.repeat(50)}`,
                  `Generated by TensorAx Studio — ${new Date().toLocaleDateString()}`,
                ].filter(Boolean).join('\n');
                const filename = `${value.projectName || 'project'}_brief.txt`;
                try {
                  const res = await fetch('/api/save-file', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ filename, data: lines, folder: `assets/0. Projects/${(value.projectName || 'project').replace(/\s+/g, '_')}` }),
                  });
                  const json = await res.json();
                  if (res.ok) {
                    alert(`Saved: ${json.path}`);
                  } else {
              ...
              title="Export brief to project folder"
            >
