...
const StyleInspirations: React.FC<{ value: GeneralDirectionType; onChange: (v: GeneralDirectionType) => void }> = ({ value, onChange }) => {
  return (
    <div className="mt-2.5">
      <label className="block text-[9px] font-bold text-[#D7D7D7]/80 uppercase tracking-wide mb-1.5">
        Style Inspirations <span className="text-[#D7D7D7]/40 font-normal">(up to 3)</span>
      </label>
      {(value.styleVideos || []).map((sv, i) => (
        <div key={i} className="mb-2 flex items-center gap-1.5">
          <span className="text-[8px] font-black text-[#E6C01F] uppercase tracking-wider w-3">{i + 1}</span>
          <input
            type="text"
            value={sv.url}
            onChange={(e) => {
            ...
            className="flex-1 bg-[#B0B0B0] border border-[#8A8A8A] rounded px-2 py-1.5 text-[9px] text-[#0d0d0d] placeholder:text-[#0d0d0d]/50 focus:ring-1 focus:ring-[#E6C01F]/50 outline-none"
          />
          <button
            onClick={() => {
      ...
      {(value.styleVideos || []).length < 3 && (
        <button
          ...
          className="w-full flex items-center justify-center gap-1.5 py-1.5 rounded-lg border border-dashed border-[#8A8A8A] text-[9px] font-bold text-[#D7D7D7]/60 hover:text-[#E6C01F] hover:border-[#E6C01F]/40 transition-colors"
        >
...
export const GeneralDirection: React.FC<GeneralDirectionProps> = ({
  ...
  return (
    <div className="flex gap-3 h-full min-h-0">
      {/* LEFT: Form */}
      <aside className="w-[38%] min-w-[280px] max-w-[420px] h-full bg-[#5A5A5A] border border-[#9A9A9A] rounded-xl flex flex-col overflow-hidden">
        <div className="p-4 border-b border-[#8A8A8A] flex-shrink-0 flex items-center justify-between">
          <h2 className="text-base font-heading font-bold text-white uppercase tracking-wide flex items-center gap-2">
            <i className="fa-solid fa-compass text-[#E6C01F]"></i>
            General Direction
          </h2>
          <div className="flex items-center gap-2">
            <button
              onClick={() => setShowKeyModal(!showKeyModal)}
              className={`p-1.5 rounded-lg transition-colors relative ${ideaApiKey ? 'text-[#E6C01F]' : 'text-red-400/70 hover:text-red-400'}`}
            ...
            <button
              ...
              className={`p-1.5 rounded-lg transition-colors ${showPromptPreview ? 'bg-[#E6C01F]/20 text-[#E6C01F]' : 'text-[#D7D7D7]/60 hover:text-[#D7D7D7]'}`}
            ...
            <button
              ...
              className="p-1.5 rounded-lg text-[#D7D7D7]/60 hover:text-[#D7D7D7] transition-colors"
            ...
            <button
              ...
              className="px-2 py-1 rounded-lg text-[8px] font-bold uppercase tracking-wider bg-[#E6C01F]/10 text-[#E6C01F]/70 hover:text-[#E6C01F] hover:bg-[#E6C01F]/20 border border-[#E6C01F]/20 transition-colors"
        ...
        {showKeyModal && (
          <div className="p-3 border-b border-[#8A8A8A] bg-[#4A4A4A] flex-shrink-0 space-y-2">
            <div className="flex items-center justify-between mb-1">
              <span className="text-[9px] font-black uppercase tracking-wider text-[#E6C01F]">
                <i className="fa-solid fa-robot text-[8px] mr-1"></i>
                Idea Factory AI
              </span>
              <button onClick={() => setShowKeyModal(false)} className="text-[#D7D7D7]/40 hover:text-[#D7D7D7] transition-colors">
                <i className="fa-solid fa-xmark text-[10px]"></i>
              </button>
            </div>
            <div>
              <label className="block text-[8px] font-bold text-[#D7D7D7]/70 uppercase tracking-wide mb-0.5">Model</label>
              <input
                type="text"
                value={ideaModel}
                onChange={(e) => setIdeaModel(e.target.value)}
                placeholder="claude-sonnet-4-6"
                className="w-full bg-[#B0B0B0] border border-[#8A8A8A] rounded px-2.5 py-1.5 text-[10px] text-[#0d0d0d] placeholder:text-[#0d0d0d]/50 focus:ring-1 focus:ring-[#E6C01F]/50 outline-none"
              />
            </div>
            <div>
              <label className="block text-[8px] font-bold text-[#D7D7D7]/70 uppercase tracking-wide mb-0.5">API Key</label>
              <p className="text-[7px] text-[#D7D7D7]/40 mb-0.5">Separate from Scenes — Claude key for creative text, Gemini key for image analysis.</p>
              <input
                type="password"
                value={ideaApiKey}
            ...
            <button
              ...
              className="w-full py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest bg-[#E6C01F] text-[#0d0d0d] hover:bg-[#d4af1c] transition-all flex items-center justify-center gap-1.5"
            >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div>
            <label className="block text-[10px] font-bold text-white uppercase tracking-wide mb-1.5 flex items-center gap-1.5">
              <i className="fa-solid fa-folder-open text-[#E6C01F] text-[9px]"></i>
              Project Name
            </label>
            <input
              type="text"
              ...
              className="w-full bg-[#B0B0B0] border border-[#8A8A8A] rounded-lg px-3 py-2 text-[11px] text-[#0d0d0d] placeholder:text-[#0d0d0d]/50 focus:ring-1 focus:ring-[#E6C01F]/50 outline-none"
            />
          </div>
          ...
          <div>
            <label className="block text-[10px] font-bold text-white uppercase tracking-wide mb-1 flex items-center gap-1.5">
              <i className="fa-solid fa-bullseye text-[#E6C01F] text-[9px]"></i>
              Project Aim
            </label>
            <p className="text-[#D7D7D7]/60 text-[9px] mb-1.5">Define the overarching goal: what should the video achieve?</p>
            <textarea
              value={value.aim}
              ...
              rows={8}
              className="w-full bg-[#B0B0B0] border border-[#8A8A8A] rounded-lg px-3 py-2 text-[11px] text-[#0d0d0d] placeholder:text-[#0d0d0d]/50 focus:ring-1 focus:ring-[#E6C01F]/50 outline-none resize-y"
            />
          </div>
          ...
          <div>
            <label className="block text-[10px] font-bold text-white uppercase tracking-wide mb-1 flex items-center gap-1.5">
              <i className="fa-solid fa-bullhorn text-[#E6C01F] text-[9px]"></i>
              Call to Action
            </label>
            <input type="text" value={value.cta} onChange={(e) => update('cta', e.target.value)} placeholder="e.g. Visit our stores, Shop now, Sign up for loyalty card" className="w-full bg-[#B0B0B0] border border-[#8A8A8A] rounded-lg px-3 py-2 text-[11px] text-[#0d0d0d] placeholder:text-[#0d0d0d]/50 focus:ring-1 focus:ring-[#E6C01F]/50 outline-none" />
          </div>

          <div>
            <label className="block text-[10px] font-bold text-white uppercase tracking-wide mb-1 flex items-center gap-1.5">
              <i className="fa-solid fa-users text-[#E6C01F] text-[9px]"></i>
              Target Audience
            </label>
            <input type="text" value={value.targetAudience} onChange={(e) => update('targetAudience', e.target.value)} placeholder="e.g. Parents of kids 0-10, expecting parents" className="w-full bg-[#B0B0B0] border border-[#8A8A8A] rounded-lg px-3 py-2 text-[11px] text-[#0d0d0d] placeholder:text-[#0d0d0d]/50 focus:ring-1 focus:ring-[#E6C01F]/50 outline-none" />
          </div>

          {/* Settings */}
          <div className="border-t border-[#8A8A8A]/40 pt-3">
            <label className="block text-[10px] font-bold text-white uppercase tracking-wider mb-2.5 flex items-center gap-1.5">
              <i className="fa-solid fa-sliders text-[#E6C01F] text-[9px]"></i>
              Settings
            </label>
            <div className="grid grid-cols-2 gap-2.5">
              <div>
                <label className="block text-[9px] font-bold text-[#D7D7D7]/80 uppercase tracking-wide mb-1">Video Type</label>
                <select value={value.videoType} onChange={(e) => update('videoType', e.target.value)} className="w-full bg-[#B0B0B0] border border-[#8A8A8A] rounded-lg px-2.5 py-1.5 text-[10px] text-[#0d0d0d] focus:ring-1 focus:ring-[#E6C01F]/50 outline-none cursor-pointer">
              ...
              <div>
                <label className="block text-[9px] font-bold text-[#D7D7D7]/80 uppercase tracking-wide mb-1">Format</label>
                <select value={value.format} onChange={(e) => update('format', e.target.value)} className="w-full bg-[#B0B0B0] border border-[#8A8A8A] rounded-lg px-2.5 py-1.5 text-[10px] text-[#0d0d0d] focus:ring-1 focus:ring-[#E6C01F]/50 outline-none cursor-pointer">
              ...
              <div>
                <label className="block text-[9px] font-bold text-[#D7D7D7]/80 uppercase tracking-wide mb-1">Duration</label>
                <select value={value.duration} onChange={(e) => update('duration', e.target.value)} className="w-full bg-[#B0B0B0] border border-[#8A8A8A] rounded-lg px-2.5 py-1.5 text-[10px] text-[#0d0d0d] focus:ring-1 focus:ring-[#E6C01F]/50 outline-none cursor-pointer">
                  <option value="1.5min">1.5 min (Short form)</option>
                  <option value="3min">3 min (Long form)</option>
                </select>
              </div>
              <div>
                <label className="block text-[9px] font-bold text-[#D7D7D7]/80 uppercase tracking-wide mb-1">Tone</label>
                <select value={value.tone} onChange={(e) => update('tone', e.target.value)} className="w-full bg-[#B0B0B0] border border-[#8A8A8A] rounded-lg px-2.5 py-1.5 text-[10px] text-[#0d0d0d] focus:ring-1 focus:ring-[#E6C01F]/50 outline-none cursor-pointer">
      ...
      <main className="flex-1 min-w-0 h-full bg-[#5A5A5A] border border-[#9A9A9A] rounded-xl flex flex-col overflow-hidden">
        <div className="p-4 border-b border-[#8A8A8A] flex-shrink-0 flex items-center justify-between">
          <h2 className="text-base font-heading font-bold text-white uppercase tracking-wide flex items-center gap-2">
            <i className={`fa-solid ${finetuneMode ? 'fa-wand-magic-sparkles' : 'fa-lightbulb'} text-[#E6C01F]`}></i>
            {finetuneMode ? 'Finetuning' : 'Idea Factory'}
          </h2>
          {finetuneMode && (
            <button
              onClick={() => setFinetuneMode(false)}
              className="text-[9px] font-bold uppercase tracking-wider text-[#D7D7D7]/60 hover:text-[#E6C01F] transition-colors flex items-center gap-1"
            >
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {finetuneMode ? (
            <div className="space-y-4">
              <div className="bg-[#4A4A4A] border border-[#E6C01F]/30 rounded-xl overflow-hidden">
                <div className="flex items-center gap-3 px-4 py-3 bg-[#3A3A3A] border-b border-[#6A6A6A]">
                  <span className="flex-shrink-0 w-7 h-7 rounded-full bg-[#E6C01F] flex items-center justify-center text-[#0d0d0d] text-xs font-black"><i className="fa-solid fa-check text-[9px]"></i></span>
                  <input
                    type="text"
                    value={finetuneTitle}
                    onChange={(e) => setFinetuneTitle(e.target.value)}
                    className="flex-1 bg-transparent text-[13px] font-bold text-white outline-none border-b border-transparent focus:border-[#E6C01F]/40"
                  />
                </div>
                <div className="p-4">
                  <textarea
                    ...
                    rows={16}
                    className="w-full bg-[#3A3A3A] border border-[#6A6A6A] rounded-lg p-3 text-[11px] text-[#D7D7D7] leading-relaxed focus:ring-1 focus:ring-[#E6C01F]/50 outline-none resize-y font-sans"
                  />
                </div>
              </div>

              <div className="bg-[#4A4A4A] border border-[#6A6A6A] rounded-xl p-4">
                <label className="block text-[10px] font-bold text-white uppercase tracking-wide mb-2 flex items-center gap-1.5">
                  <i className="fa-solid fa-pen text-[#E6C01F] text-[9px]"></i>
                  Additional Directions <span className="font-normal text-[#D7D7D7]/40">(optional)</span>
                </label>
                <textarea
                  ...
                  rows={2}
                  className="w-full bg-[#3A3A3A] border border-[#6A6A6A] rounded-lg p-3 text-[11px] text-[#D7D7D7] placeholder:text-[#8A8A8A] leading-relaxed focus:ring-1 focus:ring-[#E6C01F]/50 outline-none resize-none"
                />
                <div className="mt-3 flex gap-2">
                  <button
                    ...
                    className="flex-1 py-3 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 bg-[#E6C01F] text-[#0d0d0d] hover:bg-[#d4af1c] disabled:opacity-40 disabled:cursor-not-allowed active:scale-[0.98]"
                  >
            ...
            <div className="space-y-4">
              {(() => {
                ...
                {rejectionLog.length > 0 && (
                  <div className="bg-[#3A3A3A] border border-[#6A6A6A] rounded-lg px-3 py-2 mb-1">
                ...
                {visibleIdeas.map((idea, i) => {
                  ...
                  return (
                    <div key={i} className={`bg-[#4A4A4A] border rounded-xl overflow-hidden transition-colors ${isKept ? 'border-[#E6C01F]/40 ring-1 ring-[#E6C01F]/20' : 'border-[#6A6A6A] hover:border-[#E6C01F]/30'}`}>
                      <div className="flex items-center gap-3 px-4 py-3 bg-[#3A3A3A] border-b border-[#6A6A6A]">
                        <span className="flex-shrink-0 w-7 h-7 rounded-full bg-[#E6C01F] flex items-center justify-center text-[#0d0d0d] text-xs font-black">{isKept ? <i className="fa-solid fa-heart text-[10px]"></i> : idea.num}</span>
                        <h3 className="flex-1 text-[13px] font-bold text-white leading-snug">{idea.title || `Idea ${idea.num}`}</h3>
                        {isKept && <span className="text-[8px] font-black uppercase tracking-wider text-[#E6C01F]/60 bg-[#E6C01F]/10 px-1.5 py-0.5 rounded">kept</span>}
                        <button
                          onClick={() => {
                          ...
                          className={`p-1 rounded transition-colors ${
                            editingIdea === idea.num
                              ...
                              : 'text-[#D7D7D7]/40 hover:text-[#E6C01F]'
                          }`}
                      ...
                      <div className="p-4">
                        {editingIdea === idea.num ? (
                          <textarea
                            ...
                            rows={14}
                            className="w-full bg-[#3A3A3A] border border-[#E6C01F]/30 rounded-lg p-3 text-[11px] text-[#D7D7D7] leading-relaxed focus:ring-1 focus:ring-[#E6C01F]/50 outline-none resize-y font-sans"
                            autoFocus
                          />
                        ) : hasStructured ? (
                          <div className="space-y-2.5">
                            {parsed.map((sec, j) => (
                              <div key={j}>
                                <span className="text-[9px] font-black uppercase tracking-wider text-[#E6C01F]/70">{sec.label}</span>
                                <p className="text-[11px] text-[#D7D7D7] leading-relaxed mt-0.5">{sec.content}</p>
                              </div>
                            ))}
                          </div>
                        ) : (
                          <pre className="text-[11px] text-[#D7D7D7]/80 whitespace-pre-wrap font-sans leading-relaxed">{idea.body}</pre>
                        )}
                      </div>
                      <div className="px-4 py-2.5 bg-[#3A3A3A] border-t border-[#6A6A6A] space-y-2">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-1.5">
                              ...
                              <button
                                ...
                                className={`w-8 h-8 rounded-lg flex items-center justify-center text-base transition-all ${
                                  ratings[idea.num] === key
                                    ? 'bg-[#E6C01F]/20 scale-110 ring-1 ring-[#E6C01F]/40'
                            ...
                            <div className="w-px h-5 bg-[#6A6A6A] mx-0.5"></div>
                            <button
                              ...
                              className="w-8 h-8 rounded-lg flex items-center justify-center text-xs transition-all text-[#D7D7D7]/40 hover:text-[#E6C01F] hover:bg-[#4A4A4A] disabled:opacity-30 disabled:cursor-not-allowed"
                          ...
                          <button
                            ...
                            className="px-3 py-1.5 rounded-lg text-[9px] font-bold uppercase tracking-wider transition-all bg-[#5A5A5A] text-[#E6C01F] border border-[#E6C01F]/40 hover:bg-[#E6C01F]/10"
                          >
                        ...
                        <input
                          ...
                          className="w-full bg-[#4A4A4A] border border-[#6A6A6A] rounded px-2.5 py-1.5 text-[10px] text-[#D7D7D7] placeholder:text-[#8A8A8A] focus:ring-1 focus:ring-[#E6C01F]/50 outline-none"
                        />
                      </div>
                    </div>
                  );
                ...
                {hasFeedback && (
                  <button
                    ...
                    className="w-full py-3 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 bg-[#4A4A4A] text-[#E6C01F] hover:bg-[#5A5A5A] border border-[#E6C01F]/30 hover:border-[#E6C01F]/60 disabled:opacity-40 disabled:cursor-not-allowed active:scale-[0.98] mt-2"
                  >
            ...
            <div className="flex flex-col items-center justify-center h-full text-center">
              <i className="fa-solid fa-lightbulb text-5xl text-[#8A8A8A] mb-4"></i>
              <p className="text-[#D7D7D7]/60 text-sm font-bold uppercase tracking-widest mb-2">Idea Factory</p>
              <p className="text-[#D7D7D7]/40 text-xs max-w-md leading-relaxed">
              ...
              {activeBrand && (
                <div className="mt-4 flex items-center gap-2 bg-[#3A3A3A] px-3 py-1.5 rounded-lg border border-[#6A6A6A]">
                  <i className="fa-solid fa-tag text-[#E6C01F] text-[10px]"></i>
                  <span className="text-[10px] text-[#D7D7D7]">Brand: <strong className="text-white">{activeBrand.name}</strong></span>
                </div>
              )}
            </div>
          )}
