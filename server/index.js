...
// ── Test endpoint: competitor discovery (server-side, key in request) ────
app.post("/api/test/discover", async (req, res) => {
  try {
    const { apiKey, model, direction, platform, ownHandle, maxResults } = req.body;
    if (!apiKey || !direction || !platform) {
      return res.status(400).json({ error: "Missing apiKey, direction, or platform" });
    }
...
Given a research direction/brief and platform, identify the top ${max} ${platformName} accounts that are the most relevant competitors, benchmarks, or leading voices in this domain.
...
- If the direction is in a specific language/country, prioritise accounts from that region`;
    ...
    const competitors = (parsed.competitors || [])
      ...
      .map(c => ({ handle: c.handle.replace(/^@/, '').trim(), reasoning: c.reasoning || '', confidence: c.confidence || 0.5 }))
      .filter(c => !ownHandle || c.handle.toLowerCase() !== ownHandle.replace(/^@/, '').toLowerCase());

    res.json({ competitors });
  } catch (err) {
    console.error("[test/discover]", err);
