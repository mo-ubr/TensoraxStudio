...
function exportExcel(lang) {
  ...
  function addSectionsAsSheets(sectionId, prefix, maxSheets) {
    ...
    if (cards.length === 0) {
      const rows = [];
      el.querySelectorAll('h2, h3, p, li').forEach(n => {
        rows.push({ Content: n.innerText.trim() });
      });
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(rows.length ? rows : [{ Content: 'Empty' }]), prefix.trim());
      return;
    }
    cards.forEach((card, i) => {
      if (sheetCount >= (maxSheets || 10)) return;
      const heading = card.querySelector('h2, h3');
      const title = heading ? heading.innerText.trim() : 'Section ' + (i+1);
      let name = (prefix + (i+1) + ' ' + title).replace(/[\\/*?:\[\]]/g, '').substring(0, 31);
      cardToSheet(card, wb, name);
      sheetCount++;
    });
  }

  addSectionsAsSheets('sec-analysis', 'A', 8);
  addSectionsAsSheets('sec-recommendations', 'R', 12);
