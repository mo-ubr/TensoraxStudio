...
function renderOverview() {
  ...
  html += '<table><tr><th style="width:30px">#</th><th style="min-width:150px">Account</th><th style="width:80px">Followers</th><th style="width:55px">' + CFG.contentLabel + '</th><th style="width:85px">Total ' + CFG.viewsLabel + '</th><th style="width:85px">Total ' + CFG.likesLabel + '</th><th style="width:80px">Avg ' + CFG.viewsLabel + '</th></tr>';
...
function buildContentTable(data) {
  let html = '<table><tr><th style="width:56px">Thumb</th><th style="width:120px">Author</th><th style="min-width:200px">Text</th><th style="width:65px">' + CFG.viewsLabel + '</th><th style="width:65px">' + CFG.likesLabel + '</th><th style="width:55px">' + CFG.sharesLabel + '</th><th style="width:55px">' + CFG.commentsLabel + '</th><th style="width:50px">Eng %</th><th style="width:80px">Date</th><th style="width:40px">Link</th></tr>';
  data.forEach(p => {
...
function exportExcel(lang) {
  ...
  // 7 & 8. Analysis + Recommendations — each card/section becomes its own Excel sheet
  // 7 & 8. Analysis + Recommendations — each card becomes its own sheet
  // Walk the DOM in order to interleave headings, text, and tables correctly
  function cardToSheet(card, wb, sheetName) {
    // Collect text items and tables in document order
    const textBefore = []; // text before the first table
    const tables = [];     // [{headers, rows}]
    const textAfter = [];  // text after last table
    let seenTable = false;

    const nodes = card.querySelectorAll('h2, h3, p, li, table, ul');
    nodes.forEach(node => {
      if (node.tagName !== 'TABLE' && node.closest('table')) return;
      if (node.tagName === 'LI' && node.parentElement && node.parentElement.tagName === 'UL') return;

      if (node.tagName === 'TABLE') {
        seenTable = true;
        const headers = [...node.querySelectorAll('th')].map(th => th.innerText.trim());
        const tRows = [];
        node.querySelectorAll('tr').forEach(tr => {
          const cells = [...tr.querySelectorAll('td')];
          if (cells.length === 0) return;
          const row = {};
          cells.forEach((td, i) => { row[headers[i] || ('Col'+(i+1))] = td.innerText.trim(); });
          tRows.push(row);
        });
        tables.push({ headers, rows: tRows });
        return;
      }
      ...
      if (node.tagName === 'UL') {
        ...
        text = [...node.querySelectorAll('li')].map(li => '• ' + li.innerText.trim()).join('; ');
      } else if (node.tagName === 'H2' || node.tagName === 'H3') {
        text = '▌ ' + node.innerText.trim().toUpperCase();
      } else if (node.tagName === 'P') {
        if (node.closest('table')) return;
        text = node.innerText.trim();
      }
      if (!text) return;
      if (!seenTable || tables.length === 0) textBefore.push(text);
      else textAfter.push(text);
    });

    // Build the sheet: use the FIRST table as the primary structure
    // Put text above and below it
    const ws = XLSX.utils.aoa_to_sheet([]);
    let currentRow = 0;

    // Text before table
    if (textBefore.length > 0) {
      const aoa = textBefore.map(t => [t]);
      XLSX.utils.sheet_add_aoa(ws, aoa, { origin: { r: currentRow, c: 0 } });
      currentRow += textBefore.length + 1; // +1 for blank row
    }

    // Tables — each as proper columnar data
    tables.forEach((table, ti) => {
      if (table.rows.length === 0) return;
      // Header row
      XLSX.utils.sheet_add_aoa(ws, [table.headers], { origin: { r: currentRow, c: 0 } });
      currentRow++;
      // Data rows
      const dataAoa = table.rows.map(row => table.headers.map(h => row[h] || ''));
      XLSX.utils.sheet_add_aoa(ws, dataAoa, { origin: { r: currentRow, c: 0 } });
      currentRow += dataAoa.length + 1; // +1 blank after table
    });

    // Text after table
    if (textAfter.length > 0) {
      const aoa = textAfter.map(t => [t]);
      XLSX.utils.sheet_add_aoa(ws, aoa, { origin: { r: currentRow, c: 0 } });
    }

    // Auto-size column widths based on content
    const maxCols = Math.max(1, ...tables.map(t => t.headers.length), 1);
    const colWidths = [];
    for (let c = 0; c < maxCols; c++) {
      let maxLen = 10;
      // Check headers
      tables.forEach(t => { if (t.headers[c]) maxLen = Math.max(maxLen, t.headers[c].length); });
      // Check data
      tables.forEach(t => t.rows.forEach(r => {
        const val = Object.values(r)[c] || '';
        maxLen = Math.max(maxLen, String(val).length);
      }));
      // Check text rows
      if (c === 0) textBefore.concat(textAfter).forEach(t => { maxLen = Math.max(maxLen, Math.min(t.length, 80)); });
  ...
  function sectionToSingleSheet(sectionId, sheetName) {
    const el = document.getElementById(sectionId);
    if (!el) {
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet([{
        Content: isEN ? 'Not yet generated' : 'Все още не е генериран'
      }]), sheetName);
      return;
    }
    ...
    // Walk all cards sequentially onto one sheet
    const cards = el.querySelectorAll('.card');
    const sources = cards.length > 0 ? cards : [el];

    sources.forEach((card, ci) => {
      // Add blank row between cards (not before the first)
      if (ci > 0) { row += 2; }
      ...
      nodes.forEach(node => {
        ...
        if (node.tagName === 'H2' || node.tagName === 'H3') {
          row++;
          XLSX.utils.sheet_add_aoa(ws, [['\\u25CC ' + node.innerText.trim().toUpperCase()]], { origin: { r: row, c: 0 } });
          row++;
          return;
        }
        if (node.tagName === 'P') {
          if (node.closest('table')) return;
          XLSX.utils.sheet_add_aoa(ws, [[node.innerText.trim()]], { origin: { r: row, c: 0 } });
          row++;
          return;
        }
        if (node.tagName === 'UL') {
          ...
          [...node.querySelectorAll('li')].forEach(li => {
            XLSX.utils.sheet_add_aoa(ws, [['  \\u2022 ' + li.innerText.trim()]], { origin: { r: row, c: 0 } });
            row++;
          });
          return;
        }
        if (node.tagName === 'TABLE') {
          row++;
          const headers = [...node.querySelectorAll('th')].map(th => th.innerText.trim());
          if (headers.length > maxCols) maxCols = headers.length;
          // Header row
          ...
          node.querySelectorAll('tr').forEach(tr => {
            const cells = [...tr.querySelectorAll('td')];
            if (cells.length === 0) return;
            XLSX.utils.sheet_add_aoa(ws, [cells.map(td => td.innerText.trim())], { origin: { r: row, c: 0 } });
            row++;
          });
          row++;
          return;
    ...
    // Auto-size columns
    ws['!cols'] = Array.from({ length: maxCols }, (_, i) => ({ wch: i === 0 ? 45 : 25 }));

    try { XLSX.utils.book_append_sheet(wb, ws, sheetName); } catch(e) {}
  }

  sectionToSingleSheet('sec-analysis', isEN ? 'Analysis' : 'Анализ');
  sectionToSingleSheet('sec-recommendations', isEN ? 'Recommendations' : 'Препоръки');
